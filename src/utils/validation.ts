import { ALLOWED_WORD_EXTENSIONS, ALLOWED_WORD_MIME_TYPES, ALLOWED_PDF_EXTENSIONS, ALLOWED_PDF_MIME_TYPES } from '../types/document';
 ...
 */
export function validatePdfDocument(file: File | null): ValidationResult {
    if (!file) {
        return { valid: false, error: 'Please select a file' };
    }
    ...
    if (!hasValidExtension) {
        return {
            valid: false,
            error: `Invalid file format. Allowed formats: ${ALLOWED_PDF_EXTENSIONS.join(', ')}`,
        };
    }

    // Check MIME type (as additional validation)
    const hasValidMimeType = ALLOWED_PDF_MIME_TYPES.includes(
        file.type as (typeof ALLOWED_PDF_MIME_TYPES)[number]
    );
